...
**ARBITRAGE du gardien-DPO.** La conservation au vivier est jugée **nécessaire et proportionnée** à la finalité, au prix d'une **minimisation stricte** — aucune alternative réaliste n'atteint la finalité avec moins de données :
...
**Le risque est nommé honnêtement** : une **synthèse d'entretien est une appréciation** — donc une donnée personnelle **accessible au candidat** au titre du droit d'accès. Mal rédigée ou trop longtemps conservée, elle pourrait exposer un jugement au-delà de son contexte, ou être perçue comme une note de réputation.

Atténuations portées par le cadre parent :
...
## 6. Conclusion AIPD (art. 35) — pas d'analyse d'impact distincte requise

**ARBITRAGE du gardien-DPO (option A).** Le traitement de recrutement **ne réunit aucun** des marqueurs qui rendent une AIPD typiquement obligatoire :

- **pas de surveillance systématique** (à la différence de l'écoute Teams/Claude, qui a justifié sa propre AIPD) ;
- **pas de décision entièrement automatisée** produisant des effets juridiques (art. 22) — la décision de recrutement reste **humaine** (cadre parent §1) ;
- **pas de profilage** ni de **données sensibles** (exclues en dur — cadre parent §1, §4) ;
- **volume faible**, firme solo aujourd'hui.

En conséquence, le **risque résiduel est MAÎTRISÉ et NON ÉLEVÉ** au sens de l'art. 35 : **aucune AIPD distincte n'est requise** pour ce traitement. Cette conclusion **ferme la case §11.2** du cadre parent (`rgpd-recrutement-candidats.md`) ; l'**alignement de l'en-tête du parent** (mention de §11.2 tranchée) est un **candidat ultérieur**, scope séparé.

Cette conclusion est **conditionnée au maintien des mesures** des §2–§3 et **révisée au même rythme que le §5** (annuel ; à chaque évolution du processus). Si le périmètre évoluait vers du profilage, un tri automatisé ou un volume massif, l'AIPD serait **réexaminée**.

## 7. Comment cette annexe évolue

Annexe normative — elle évolue par la **boucle de promotion** (`doctrine.md` §7) : candidat, avis d'impact, décision du gardien attachée à l'empreinte, promotion, retour arrière par repointage. Sa promotion rend la **phase vivier** (6.1.f) **opérable** ; elle reste subordonnée au respect des conditions du §5.
