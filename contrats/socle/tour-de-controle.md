...
> **Version** : 2.5 — *candidat*. **Statut** : contrat socle — fait foi.
> **Domicile** : `contrats/socle/tour-de-controle.md`. **Autorité de promotion** : gardien du temple.
> **Changelog** :
> v2.5 — candidat, 5 août 2026 (arbitrage gardien du 05/08/2026 — « dès que la ligne CRM passe en Gagnée, la machine crée la ligne mission ; je dépose ensuite l'Excel à mon rythme, en me basant sur le numéro attribué ») : **§3, bandeau 2** — la cascade « Gagnée » ouvre désormais l'**OSSATURE** de la mission et non plus seulement son code et son espace. Écritures de la cascade, **énumérées et exhaustives**, dans cet ordre : (1) `allouer_code_mission` (T-0038, cran notifié, inchangé) ; (2) **`creer_ligne_mission` — NOUVEAU** : UNE ligne dans la Liste « Missions », six valeurs **dérivées** (`Title` = « Projet » + nom de l'opportunité, `NomClient` = nom du compte résolu depuis le lookup, statut « En cours », `DateDebut` = 1er du mois de la bascule, `DateFin` = 31 décembre de l'année de la bascule, `CodeMission` = l'entier tout juste alloué), cible figée côté serveur (`GRAPH_MISSIONS_LIST_ID`), préconditions fail-closed, cran **notifié** — `modele-donnees.md` **v1.31 §2 bis** et `table-des-crans.yaml` **v1.18**, qui font foi ; (3) création de l'espace de mission (inchangée). La machine pose un **état initial cohérent**, elle ne le fige pas : les six valeurs restent éditables à la main. **Le classeur de saisie SORT de la cascade** — il demeure un **dépôt humain**, au rythme du gardien, nommé d'après le `CodeMission` alloué (`modele-donnees.md` §5.6) : l'invariant « la machine n'écrit jamais la saisie » en sort **renforcé**, pas contourné (chantier **T-0040 requalifié** en conséquence ; T-0042 quitte son chemin critique, sans être lui-même requalifié). **STOP au premier incident** : un échec après l'allocation affiche l'état honnêtement — jamais de réparation silencieuse ni de reprise automatique — et le code alloué n'est **jamais recyclé**. **Motif** : « Missions actives » se calcule sur la **Liste « Missions »**, qu'**aucun geste outillé n'alimentait** — trou de chaîne exposé par l'usage du 05/08/2026 (la mission 4, « Cockpit M365 », vivait au CRM — Gagnée le 24/07, `CodeMission = 4` alloué par la machine — et à l'économique, sans aucune ligne dans la Liste « Missions » ; rattrapée à la main le 05/08 sous identité déléguée du gardien). Aucun autre bandeau, aucun régime, aucun cran existant modifié ; aucune entrée de changelog antérieure réécrite.
> v2.4 — candidat, 4 août 2026 : correctif de RÉFÉRENCE, **sans changement normatif** (patron de `modele-donnees.md` v1.24) — §3 bandeau 4, la source du Budget porte désormais ses coordonnées RÉELLES au tenant : le classeur se nomme `budgetentreprise.xlsx` (**sans tiret**) et vit sur le site **`AlliaConsulting-Contratsetadministratif`**, dossier **« 09 - Budget Allia »** — et non sur ManagementetGestion, qui est le site de la couche de SAISIE (`modele-donnees.md` §5.6) et non celui du budget. Les deux références étaient fausses dans la même parenthèse ; faits mesurés sous identité déléguée le 04/08/2026, adresse probante (driveId / itemId) gravée en `modele-donnees.md` **v1.30 §5.7**, qui fait foi. **Aucune règle, aucun régime, aucun bandeau, aucun cran modifié** ; l'entrée v2.3 ci-dessous conserve l'ancienne graphie — historique immuable.
...
- **Hiérarchie des bandeaux (ordre figé, priorité décroissante)** :
  ...
  2. **Pipe commercial** — compteurs : propositions en cours, montant proposé, **pipe pondéré**
     ...
     **info-bulle** du bandeau. La table des opportunités ne vit plus sur la première page :
     ...
     Prospect, à confirmer au premier usage réel) ; passage en Gagnée → **cascade déterministe**
     (annonce exhaustive des écritures + confirmation explicite, §1 régime 2) qui ouvre
     l'**ossature** de la mission, dans cet ordre : (1) **`allouer_code_mission`** — le
     `CodeMission` alloué et réécrit sur l'opportunité (T-0038, cran notifié) ;
     (2) **`creer_ligne_mission`** — **une ligne dans la Liste « Missions »**, aux valeurs
     **dérivées** (`Title` = « Projet » + nom de l'opportunité ; `NomClient` = nom du compte
     résolu depuis le lookup ; statut **« En cours »** ; `DateDebut` = **1er du mois** de la
     bascule ; `DateFin` = **31 décembre de l'année** de la bascule ; `CodeMission` = l'entier
     tout juste alloué) — un **état initial cohérent**, jamais figé : les six valeurs restent
     **éditables à la main** ensuite (`modele-donnees.md` §2 bis, `table-des-crans.yaml`) ;
     (3) création de l'**espace de mission**. **Le classeur de saisie n'entre pas dans la
     cascade** : c'est un **dépôt humain**, au rythme du gardien, nommé d'après le `CodeMission`
     alloué (`modele-donnees.md` §5.6) — l'invariant « la machine n'écrit jamais la saisie »
     en sort renforcé. **STOP au premier incident** : si une écriture échoue après l'allocation,
     l'état est affiché honnêtement, sans réparation silencieuse ni reprise automatique ; le
     code alloué n'est **jamais recyclé** (il reste porté par l'opportunité) et la ligne
     Missions se rattrape par un geste explicite.
  ...
  4. **Rentabilité et résultats** — tableau **de l'année en cours** : douze mois × (Budget |
     ...
     `budgetentreprise.xlsx` (table `T_Budget` ; site Contratsetadministratif, dossier
     « 09 - Budget Allia » — `modele-donnees.md` §5.7 ; saisie gardien — jamais
