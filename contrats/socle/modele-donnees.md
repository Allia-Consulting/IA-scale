...
> **Version** : 1.16 — *candidat*. **Statut** : contrat socle — fait foi.
...
| Compte / Client | référentiel des clients | Liste « Comptes » *(schéma complet : § 2 bis)* | id compte | source |
| Contact | interlocuteur chez un compte | Liste « Contacts » *(schéma complet : § 2 bis)* | id contact | source · **données personnelles (RGPD)** |
| Opportunité | affaire potentielle (pipeline commercial) | Liste « Opportunites » *(schéma complet : § 2 bis)* | id opportunité | source / dérivé |
...
| Liste « Comptes » | site AlliaConsuling / Liste Comptes | lecture/écriture selon cran | Référentiel clients (personnes morales — pas de donnée personnelle). Identifiant stable = **id compte** (`Title`, ex. `A-001`). **Colonnes** (libellé → nom interne / type) : Identifiant compte → `Title` (id stable) · Nom du compte → `NomCompte` (texte — raison sociale) · Secteur → `Secteur` (texte — libre en v1, pas de référentiel figé prématuré) · Site web → `SiteWeb` (texte, optionnel). Noms internes : voir **§2 ter**. **Liste existante au tenant (vide)** — mise en conformité au schéma = runbook humain (`T-0026`). |
| Liste « Contacts » | site AlliaConsuling / Liste Contacts | **accès restreint** | ⚠️ **données personnelles (RGPD)** — contact professionnel B2B (base : intérêt légitime — prospection et relation d'affaires ; minimisation : aucune donnée hors sphère professionnelle ; opposition honorée par suppression de la fiche). Identifiant stable = **id contact** (`Title`, ex. `CT-001`). **Colonnes** (libellé → nom interne / type) : Identifiant contact → `Title` (id stable) · Nom du contact → `NomContact` (texte) · Compte → `Compte` (**lookup → Comptes** sur `Title` — le rattachement) · Fonction → `Fonction` (texte) · Email → `Email` (texte) · Téléphone → `Telephone` (texte, optionnel). Noms internes : voir **§2 ter**. **Liste à créer au tenant** (runbook humain, `T-0026`, principe de création §2 ter). **Journalisation à activer** avant accès agent réel (cf. T-0003). |
| Liste « Opportunites » | site AlliaConsuling / Liste Opportunites | lecture/écriture selon cran (dérivés via Zone-de-proposition) | Libellé affiché « Opportunités » (nom interne ASCII, principe §2 ter). Pipeline commercial. Identifiant stable = **id opportunité** (`Title`, ex. `O-001`). **Colonnes** (libellé → nom interne / type) : Identifiant opportunité → `Title` (id stable) · Nom de l'opportunité → `NomOpportunite` (texte — intitulé court) · Compte → `Compte` (**lookup → Comptes** sur `Title` — le rattachement) · Contact principal → `ContactPrincipal` (**lookup → Contacts** sur `Title`, optionnel) · Étape → `Etape` (choix **unique** : Détectée / Qualifiée / Proposition / Négociation / Gagnée / Perdue — pas de défaut, pas d'ajout manuel) · Montant → `Montant` (nombre — € HT estimé) · Échéance → `Echeance` (date — signature attendue) · Responsable commercial → `ResponsableCommercial` (personne, unique) · Code mission → `CodeMission` (texte — **le lien Opportunité → Mission** : renseigné au passage à « Gagnée » avec le code de la mission créée ; référence par identifiant stable, pas de lookup tant que la liste « Missions » n'est pas câblée). Une opportunité **détectée par un agent** naît en **Zone-de-proposition** (`Origine` = « crm-opportunite ») et ne rejoint cette liste **qu'à validation** (promotion tracée) ; la saisie humaine directe reste libre. **Liste à créer au tenant** (runbook humain, `T-0026`). |
...
**Liste « Comptes »** (identifiant stable = `Title`)

| Libellé affiché | Nom interne | Type | Notes |
|---|---|---|---|
| Identifiant compte | `Title` *(existant)* | — | id stable, ex. `A-001` |
| Nom du compte | `NomCompte` | Texte | raison sociale |
| Secteur | `Secteur` | Texte | libre en v1 — pas de référentiel figé prématuré |
| Site web | `SiteWeb` | Texte | optionnel |

**Liste « Contacts »** (identifiant stable = `Title`)

| Libellé affiché | Nom interne | Type | Notes |
|---|---|---|---|
| Identifiant contact | `Title` *(existant)* | — | id stable, ex. `CT-001` |
| Nom du contact | `NomContact` | Texte | donnée personnelle (contact professionnel B2B) |
| Compte | `Compte` | Lookup → Comptes (`Title`) | **le rattachement** |
| Fonction | `Fonction` | Texte | |
| Email | `Email` | Texte | contact professionnel |
| Téléphone | `Telephone` | Texte | optionnel |

**Liste « Opportunites »** (libellé « Opportunités » ; identifiant stable = `Title`)

| Libellé affiché | Nom interne | Type | Notes |
|---|---|---|---|
| Identifiant opportunité | `Title` *(existant)* | — | id stable, ex. `O-001` |
| Nom de l'opportunité | `NomOpportunite` | Texte | intitulé court |
| Compte | `Compte` | Lookup → Comptes (`Title`) | **le rattachement** |
| Contact principal | `ContactPrincipal` | Lookup → Contacts (`Title`) | optionnel |
| Étape | `Etape` | Choix (unique) | Détectée / Qualifiée / Proposition / Négociation / Gagnée / Perdue — pas de défaut, pas d'ajout manuel |
| Montant | `Montant` | Nombre | € HT estimé |
| Échéance | `Echeance` | Date | signature attendue |
| Responsable commercial | `ResponsableCommercial` | Personne (unique) | porteur de l'affaire |
| Code mission | `CodeMission` | Texte | **lien Opportunité → Mission** — renseigné au passage à « Gagnée » |
