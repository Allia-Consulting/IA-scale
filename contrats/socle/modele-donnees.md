...
| Liste « Zone-de-proposition » | site AlliaConsuling / Liste Zone-de-proposition | **écriture (dérivés)** | domicile concret de la zone de proposition (§3). Porte un **champ d'origine** qui qualifie chaque dérivé ; valeur **« mémoire hebdo »** = synthèse produite par le batch nocturne de **mémoire d'organisation** (contrat socle candidat `contrats/socle/memoire-organisation.md`), à valider ligne à ligne le vendredi. **Écriture via Graph Lists API — nécessite le connecteur Graph MCP en écriture (voir `T-0002b`).** En attendant : zone de proposition **simulée en local** (`zone-proposition/` ; les synthèses « mémoire hebdo » sous `zone-proposition/memoire/`). |
...
Un **champ d'origine** qualifie la provenance de chaque dérivé. La valeur **« mémoire hebdo »** est réservée aux synthèses hebdomadaires candidates produites par le batch de **mémoire d'organisation** (`contrats/socle/memoire-organisation.md`, candidat) : domicile transitoire de cette mémoire en attendant T-0002b, validées ligne à ligne le vendredi (non-validé = oublié). La zone de proposition reste **simulée en local** tant que T-0002b n'est pas déployé ; les synthèses « mémoire hebdo » y vivent sous `zone-proposition/memoire/`.

## 4. État du câblage

Les domiciles concrets (site, listes, bibliothèques) sont **partiellement câblé — session 7 juin 2026 ; écriture Graph = T-0002b** (voir §2 bis). Les **emplacements** sont renseignés (lecture documentée) ; l'**écriture** dans les listes — notamment `Zone-de-proposition` — requiert un connecteur **Graph MCP en écriture**, identifié comme chantier `T-0002b`. En attendant, la zone de proposition est **simulée en local** (`zone-proposition/`).
