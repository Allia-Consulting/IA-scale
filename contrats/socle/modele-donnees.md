...
- **Domicile.** Classeur de **saisie humaine** du budget entreprise : site
  **`AlliaConsulting-Contratsetadministratif`**, bibliothèque « Documents », dossier
  **« 09 - Budget Allia »**. Coordonnées tenant (drive-id / item-id) : **variables
  d'environnement** posées par runbook gardien, **jamais au canon** (même régime que le
  référentiel de coûts, §5.5).
- **État mesuré le 03/08/2026.** Le **dossier existe** et porte deux classeurs : `BP allia.xlsx`
  (source de modélisation du business plan — grilles de salaires par grade, TJM, plan de charge ;
  **8 onglets, aucune table nommée**) et `Business plan et tréso.xlsx` (**sensible**). Le classeur
  **`budget-entreprise.xlsx`** portant la table `T_Budget` au schéma ci-dessous **n'existe pas
  encore** : ce domicile est **NOMMÉ, à créer et renseigner par runbook gardien** — même régime
  que les Listes « CRM » (`T-0026`) et « Factures » (`T-0030`), nommées au canon avant leur
  création au tenant. Chantier consommateur : **`T-0051`**.
...
- **La machine ne l'écrit JAMAIS.** Droit **`read`** seul, **même régime que la couche de saisie**
  (§5.6) : le budget est une **source humaine**, l'invariant « la machine n'écrit jamais la
  saisie » s'y étend par construction. **Ne jamais ouvrir d'écriture** sur ce domicile.
