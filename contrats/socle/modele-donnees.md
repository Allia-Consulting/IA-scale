...
> **Version** : 1.33 — *candidat*. **Statut** : contrat socle — fait foi.
...
> **Changelog** : v1.33 — **candidat**, 6 août 2026 (S50, arbitrages gardien du 06/08/2026 — règle de numérotation des factures et domicile de la vérité) : **§2 bis, bloc AJOUTÉ après le tableau ; la ligne « Liste « Factures » » n'est PAS réécrite — corrigée par renvoi.** **(i) RÈGLE TRANCHÉE** : toute facture émise — **y compris un avoir, une refacturation ou une réécriture** de la même prestation — reçoit un numéro **NEUF** ; ce qui doit tenir est la **logique** (max de l'année + 1) et l'**ORDRE**, jamais la réutilisation. **(ii) DEUX ÉCARTS CANON/CODE, symétriques, mesurés par lecture du code le 06/08** : le canon décrit une précondition qui **n'existe pas** (`allouer_num_facture` ne lit AUCUN statut et ne vérifie AUCUNE ligne préexistante — il **crée** l'élément avec `Statut = émise` et `DateEmission` côté serveur) ; le code porte une **idempotence par clé `(CodeMission, EtiquetteLocale)` que le canon n'a JAMAIS décrite** et qui **contredit (i)** — une réémission voulue recevrait l'ANCIEN numéro, silencieusement. **(iii) CORRECTIF D'ARCHITECTE** : l'idempotence **change de clé**, elle ne disparaît pas — elle porte désormais sur le **GESTE** (`CleEmission`, identifiant de la confirmation) et non sur la prestation ; un rejeu rend le même numéro, une nouvelle confirmation en alloue un neuf. Colonne `CleEmission` à créer au registre — **runbook gardien**. **(iv) DOMICILE DE LA VÉRITÉ** : le **classeur de saisie fait foi** pour la facturation ; le registre « Factures » porte le **NUMÉRO** (séquence légale), **jamais** la vérité de ce qui est dû ; l'acte comptable (avoir, refacturation, retenue) relève du **comptable**, hors SI. **(v) Tolérance de LECTURE des statuts** de l'onglet « Facturation ». Aucun schéma existant, aucun domicile, aucun cran modifié ; aucune entrée de changelog réécrite.
> **Changelog** : v1.32 — **candidat**, 6 août 2026 (S50, arbitrage gardien du 06/08/2026 — régime d'accès du budget entreprise) : **§5.7, bloc AJOUTÉ en fin de section ; aucun tiret existant n'est réécrit.** **(i) RÉGIME D'ACCÈS RÉVISÉ** : le dossier « 09 - Budget Allia » **N'EST PAS** ouvert en lecture à la firme — l'octroi prévu au tiret « Régime d'accès » est **ABROGÉ**, jamais joué, jamais à jouer ; le classeur demeure réservé à **deux personnes**. La donnée budget reste néanmoins affichée à toute la firme au cockpit, **sans élévation de droits**, par **PROJECTION DÉRIVÉE** (patron des gabarits ERP, §5.1) : le serveur lit `T_Budget` sous identité machine et écrit une Liste « Budget » sur le site AlliaConsuling ; le cockpit lit **cette liste** sous l'identité de l'utilisateur. Schéma de la liste, §2 bis et cran de l'outil **NON gravés ici** — à l'ouverture de `T-0051`. **(ii) SÉPARATION DOCUMENTAIRE JOUÉE le 06/08** : `BP allia.xlsx` et `Business plan et tréso.xlsx` déplacés vers le dossier frère **« 10 - Business plan »** (même bibliothèque, même `driveId`, `itemId` préservés) ; « 09 - Budget Allia » ne porte plus que `budgetentreprise.xlsx`, dont l'**adresse probante du §5.7 est INCHANGÉE** (vérifiée après le geste). Motif : le tiret « Régime d'accès » **se contredisait** avec le tiret « État mesuré » de la même section — il ouvrait à la firme un dossier dont il constatait lui-même qu'il portait le BP. **(iii) CORRECTION DU DROIT** : le tiret « La machine ne l'écrit JAMAIS » est **FAUX sur son état du droit** — `architecture.md` porte depuis `T-0031` l'octroi `Sites.Selected` **`role=write`** de l'identité managée sur Contrats-et-administratif, corroboré par l'usage (gabarits ERP créés et régénérés sur ce site depuis le 17/07 ; `inscrire_cout_structure` y écrit `referentiel-structure.xlsx` depuis le 01/08). L'invariant « la machine n'écrit jamais le budget » est donc garanti **par le CODE** (primitives à cible figée), **PAS par le droit**. Cause de l'erreur du 04/08 : aucune primitive du serveur ne pointe sur ce classeur — **absence d'OUTIL prise pour absence de DROIT**. Aucune autre section, aucun domicile, aucun schéma modifié ; aucune entrée de changelog existante réécrite.
> **Changelog** : v1.31 — **candidat**, 5 août 2026 (S49, arbitrage gardien du 05/08/2026 — cascade d'ouverture de mission ; aligné sur `tour-de-controle.md` **v2.5** et `table-des-crans.yaml` **v1.18**) : **deux objets, distincts et nommés comme tels**. **(i) CORRECTIF DE POINTEUR — défaut de la PR #298** : l'en-tête de ce contrat portait encore « 1.29 — *promu* » alors que le **corps** portait déjà le contenu **v1.30** (§5.7 coordonnées probantes du classeur budget, §2 bis colonne `CodeCouleur`) et que l'entrée de changelog v1.30 était bien présente — seul le **pointeur de version** était resté en arrière. Il est remplacé ici ; **l'entrée v1.30 n'est pas touchée** (historique immuable) et **aucun contenu de corps n'est modifié par ce correctif**. **(ii) OUVERTURE BORNÉE DE L'ÉCRITURE AGENT SUR LA LISTE « MISSIONS »** : **§2 bis**, la ligne « Liste « Missions » » quitte le générique « lecture/écriture selon cran » pour le patron **exact** de la ligne « CRM » (v1.24) — écriture agent **bornée à l'outil serveur `creer_ligne_mission`** (T-0040), cible **figée** côté serveur (`GRAPH_MISSIONS_LIST_ID`, jamais choisie par l'appelant), **six colonnes énumérées et aucune autre** (`Title`, `CodeMission`, `NomClient`, `EnCours_x002f_Termin_x00e9_e_x00`, `DateDebut`, `DateFin`), préconditions **fail-closed** (`Etape = Gagnée` ET `CodeMission` renseigné ET **aucune ligne ne portant déjà ce `CodeMission`** — sinon refus, zéro écriture, doublon impossible par construction), cran **notifié**. Toutes les valeurs sont **dérivées**, aucune saisie humaine ; la machine pose un **état initial cohérent** qu'elle ne **fige** pas — les six valeurs restent **éditables à la main** ensuite. Le **schéma réel** de la liste est consigné, **relevé au tenant le 05/08/2026** — dont l'**asymétrie** du champ de statut : nom interne `EnCours_x002f_Termin_x00e9_e_x00`, libellé « En Cours / Terminée / Suspendue », **valeurs de choix réelles `En cours` / `Terminée` / `Suspendu`**. Geste **gouverné, borné et tracé** sur le modèle de `allouer_code_mission` (organisation.md §5) — ce n'est **pas** une écriture source libre ; **aucune autre colonne d'aucune source** n'est ouverte à l'écriture agent. Le **classeur de saisie** (§5.6) reste **hors cascade** : dépôt **humain**, au rythme du gardien, nommé d'après le `CodeMission` alloué — l'invariant « la machine n'écrit jamais la saisie » en sort **renforcé**. **Motif** : « Missions actives » se calcule sur cette liste, qu'aucun geste outillé n'alimentait — trou de chaîne exposé par l'usage du 05/08/2026 (mission 4 vivante au CRM et à l'économique, absente de la Liste « Missions » ; rattrapée à la main le jour même sous identité déléguée du gardien). Aucun autre domicile, aucun autre schéma de colonne modifié ; aucune entrée de changelog existante réécrite.
> **Changelog** : v1.30 — **candidat**, 3 août 2026 (point d'étape cockpit v2.3 du 03/08, arbitrages gardien ; aligné sur `tour-de-controle.md` **v2.3**, PR #297) : **(a) §2 bis, Liste « Ressources-Profil »** — nouvelle colonne **Code couleur → `CodeCouleur`** (choix **unique** : Rouge / Orange / Vert — sans valeur par défaut, sans ajout manuel) : **appréciation LIBRE du gardien**, **aucune règle de calcul**, écrite depuis la page **Détails staffing** du cockpit sous l'**identité de l'utilisateur** (`tour-de-controle.md` v2.3 §1/§3) ; **JAMAIS écrite par un agent** — l'accès agent de la liste reste **lecture**. **(b) §2 bis + §2 ter, Liste « Candidats »** — deux nouvelles colonnes : **Identité Entra → `IdentiteEntra`** (texte, UPN — renseignée quand elle existe) et **Disponibilité → `Disponibilite`** (date). Données personnelles candidat : cadre `rgpd-recrutement-candidats.md` **inchangé**, rétention **2 ans inchangée** (horloge « Créé le »), pertinence au recrutement. Le formulaire cockpit ne collecte **PLUS de code mission** (on recrute **sans mission** ; l'affectation naît du dépôt du classeur de saisie, §5.6). **(c) §5.7 (nouvelle) — domicile du budget entreprise**, classeur de **SAISIE HUMAINE** : site `AlliaConsulting-Contratsetadministratif`, bibliothèque « Documents », dossier **« 09 - Budget Allia »**. **RÉGIME D'ACCÈS (arbitrage gardien du 03/08)** : le **SITE** est à audience restreinte parce qu'il héberge aussi `Business plan et tréso.xlsx` (**SENSIBLE** — les deux sources ne sont **pas séparées** dans l'architecture documentaire), mais les **DONNÉES du budget ne sont PAS sensibles** : elles s'affichent à **toute la firme** au cockpit, sans mesure de protection à la lecture. Conséquence **sans élévation de droits** : l'accès **LECTURE du seul dossier « 09 - Budget Allia »** est ouvert à la firme par **geste gardien** (héritage rompu, octroi lecture — **runbook tenant**) ; le reste du site, **BP & Tréso compris, demeure restreint**. Table nommée **`T_Budget`** (`Mois` [1er du mois] | `CA_Budget` | `MasseSalariale_Budget` [part variable incluse, prorata mensuel] | `CoutsStructure_Budget` | `EBITDA_Budget` [**formule, jamais saisi**]) ; **une ligne par mois** ; mois sans budget = montants **VIDES** (jamais zéro inventé) ; lu par le cockpit (bandeau 4, colonne Budget, filtré sur l'année affichée — année en cours + bouton « Année N+1 ») **sous l'identité de l'utilisateur connecté**, sans élévation ; la **machine ne l'écrit JAMAIS** (droit `read`, même régime que la saisie). **FAIT MESURÉ** : le classeur est **DÉPOSÉ** (03/08/2026, geste gardien) et **relu sous identité DÉLÉGUÉE le 04/08/2026** — **nom réel `budgetentreprise.xlsx`** (sans tiret), **2 onglets** (`Lisez-moi`, `Budget`), table `T_Budget` **peuplée sur 36 mois** (2026-01 → 2028-12 ; les premiers mois de 2026 restent **vides** par convention), `EBITDA_Budget` **bien en formule** (`=IF(COUNT(…)=0,"",CA−MS−CS)`), part variable **incluse** dans `MasseSalariale_Budget` (contrôle janv-2027 : MS 97 987 € / EBITDA 23 108 €). **Adresse probante GRAVÉE** au §5.7 (driveId / itemId — adressage **par item ID, jamais par chemin**). **Le serveur Workbook (identité MACHINE) ne voit pas ce site** — **attendu et non bloquant** : aucun octroi machine n'est requis, le cockpit lit sous **identité utilisateur** et toute vérification de ce classeur se fait sous **identité déléguée**. **Écart de pointeur consigné** : `tour-de-controle.md` v2.3 (promu) écrit `budget-entreprise.xlsx` ; **le nom réel au tenant fait foi** — correction de pointeur à porter en micro-PR dédiée à la prochaine ouverture de `tour-de-controle.md` (non traitée ici). **Reste au runbook gardien** : l'octroi **LECTURE** du dossier « 09 - Budget Allia » à la firme (héritage rompu ; le site demeure restreint pour le BP & Tréso). Le **Réalisé** du bandeau 4 reste dérivé des **gabarits actifs + référentiel de coûts** (§5.4) — **inchangé**. **(d) §5.6 — ALIGNEMENT sur `tour-de-controle.md` v2.3** : la règle « l'humain n'édite l'économique que dans la saisie, PAS depuis le cockpit » gagne l'**exception BORNÉE** de la cascade d'émission de facture (v2.3 §1/§3 bandeau 5) — statut « émise » écrit par un **geste humain confirmé** depuis le cockpit, **sous l'identité du cliqueur, jamais machine** ; le **DOMICILE exact** de cette écriture (saisie sous identité déléguée vs registre « Factures » re-dérivé) sera **tranché à l'ouverture de T-0052**. L'invariant « **la machine n'écrit jamais la saisie** » est **INCHANGÉ**. Aucun domicile existant modifié ; aucune entrée de changelog réécrite.
> **Changelog** : v1.29 — **promu**, 1er août 2026 (S46, `T-0046` — arbitrages gardien du 01/08 au soir, institution du **rituel des temps**) : **§5.6** — ajout d'**UN** tiret « **Rituel des temps** », placé immédiatement après le bloc « Saisie (surface équipe, source humaine) ». Cadence **MENSUELLE** (l'option hebdomadaire est écartée) : à la clôture de chaque mois, **au plus tard le 2e jour ouvré du mois suivant**, chaque collaborateur affecté à une mission déclare ses **jours RÉALISÉS** du mois écoulé dans `SAISIE_Realise_<millésime>` — **sur SA ligne** (colonne `Ressource` = son **identifiant §5.3**, le même que `T_Ressources`), **à la POSITION du mois** (cellule = cumul du mois, `TOTAL` recalculé par le classeur). La déclaration est une **SAISIE SOURCE** : la machine ne l'écrit **jamais** (droit `read`) ; la **dérivation** vers `T_Imputations` la **suit**, sur déclenchement manuel du gardien, et les lignes dérivées naissent « à valider » (§5.2). **V1 MANUELLE** — aucun rappel, aucun contrôle de complétude. **NOMMÉS, NON CONSTRUITS** (`T-0046`) : l'**orchestration** du rituel et un **écran de déclaration de temps** (fonction des missions et du prévisionnel du collaborateur), qui écrirait sous l'**identité déléguée du déclarant** — jamais l'identité machine — et exigera une évolution de `tour-de-controle.md` (§1/§4, cockpit en lecture seule sur l'économique). **Aucune règle de dérivation, aucun schéma (14 colonnes), aucun domicile, aucun droit d'accès modifié** : le rituel **s'ajoute**, il ne réécrit rien. Aucune entrée de changelog existante réécrite ; aucune autre section modifiée. v1.28 — **promu**, 1er août 2026 (S46, arbitrage « merge vaut promotion », doctrine §7 note d'exécution) : promotion constatée au merge de la PR #286, épreuve verte du 01/08 au journal ; corps byte-identique au candidat ; aucune entrée existante réécrite.
...
|---|---|---|---|
| Liste « Missions » | site AlliaConsuling / Liste Missions | lecture selon cran ; **écriture agent : bornée au SEUL outil serveur `creer_ligne_mission`** (T-0040) — **création d'UNE ligne** à la bascule « Gagnée » d'une opportunité (cascade du cockpit, `tour-de-controle.md` §3 bandeau 2) : cible **figée** côté serveur (`GRAPH_MISSIONS_LIST_ID`, jamais choisie par l'appelant), **six colonnes énumérées** (`Title`, `CodeMission`, `NomClient`, `EnCours_x002f_Termin_x00e9_e_x00`, `DateDebut`, `DateFin`) et **aucune autre**, préconditions **fail-closed** (`CRM.Etape = Gagnée` ET `CodeMission` renseigné ET **aucune ligne de cette liste ne portant déjà ce `CodeMission`** — sinon refus explicite, zéro écriture ; le doublon est impossible par construction), cran **notifié** (`table-des-crans.yaml`). Ce n'est pas une écriture source libre mais un geste **gouverné, borné et tracé**, miroir de `allouer_code_mission` (organisation.md §5) — l'agent ne choisit ni la liste ni les colonnes. Aucune AUTRE colonne d'aucune source n'est ouverte à l'écriture agent | source des affaires (entité **Mission**). Identifiant stable = **code mission** (`CodeMission`, entier ≥ 1 — §2 bis « CRM », la couture). **Colonnes** (libellé → nom interne / type — **noms internes relevés au tenant le 05/08/2026**) : Titre → `Title` (texte — nom de la mission) · Code mission → `CodeMission` (entier positif, porte l'identifiant stable de la Mission) · Nom du client → `NomClient` (texte) · En Cours / Terminée / Suspendue → `EnCours_x002f_Termin_x00e9_e_x00` (choix — **valeurs réelles : `En cours` / `Terminée` / `Suspendu`** ; noter l'**asymétrie** entre le libellé de la colonne et ses valeurs de choix, relevés tels quels) · Date de début → `DateDebut` (date) · Date de fin → `DateFin` (date). **Valeurs posées par `creer_ligne_mission` — toutes DÉRIVÉES, aucune saisie humaine** : `CodeMission` = l'entier que vient d'allouer `allouer_code_mission` · `Title` = « Projet » + `CRM.NomOpportunite` · `NomClient` = `Comptes.NomCompte` résolu depuis le lookup `CRM.Compte` · statut = `En cours` · `DateDebut` = **1er du mois** de la bascule · `DateFin` = **31 décembre de l'année** de la bascule. La machine pose un **état initial cohérent** ; elle ne le **fige** pas — les six valeurs restent **éditables à la main** ensuite. Le **classeur de saisie** de la mission n'entre **PAS** dans cette cascade : c'est un **dépôt humain**, au rythme du gardien, nommé d'après le `CodeMission` alloué (**§5.6**) — l'invariant « la machine n'écrit jamais la saisie » en sort renforcé. |
...
| Liste « Ressources-Profil » | site AlliaConsuling / Liste Ressources-Profil | **lecture** (agents ; l'écriture **HUMAINE** de la seule colonne `CodeCouleur` depuis le cockpit est un geste sous droits propres, `tour-de-controle.md` v2.3 §1) | profil de ressource (compétences, séniorité). **Colonne ajoutée (v1.30)** : Code couleur → `CodeCouleur` (choix **unique** : Rouge / Orange / Vert — sans défaut, sans ajout manuel) = **appréciation LIBRE du gardien**, **aucune règle de calcul** ; affectée depuis la page **Détails staffing** du cockpit (`tour-de-controle.md` v2.3 §3 bandeau 1) ; **jamais écrite par un agent**. Colonne **à poser par runbook** (`T-0048`) |
...
> **RÈGLE DE NUMÉROTATION DES FACTURES — arbitrage gardien du 06/08/2026 (S50).** Bloc **AJOUTÉ** ;
> la ligne « Liste « Factures » » du tableau ci-dessus **n'est pas réécrite** — elle est corrigée
> **par renvoi d'ici** (forme `backlog/plan.md` §18).
> **(a) Une émission = un numéro NEUF.** Toute facture émise — **y compris un avoir, une
> refacturation ou une réécriture** de la même prestation — reçoit un numéro **nouveau**. Un
> numéro n'est **jamais** réutilisé ni rendu à un second geste. Ce qui doit tenir est la
> **logique** (`NNNN` = max de l'année + 1) et l'**ORDRE** (croissant).
> **(b) La précondition décrite ci-dessus n'existe pas dans le code** (lecture du code,
> 06/08/2026). `allouer_num_facture` ne lit **aucun statut** et ne vérifie **aucune ligne
> préexistante** : ses préconditions sont des **validations d'entrée** (`code_mission` ≥ 1,
> étiquette non vide, montant > 0, dates non vides), et il **CRÉE** l'élément avec
> `Statut = « émise »` et `DateEmission` **posés côté serveur**. La formule « ligne au statut
> émise SANS NumFacture » décrit un contrôle qui n'a jamais été implémenté.
> **(c) Le code porte une idempotence que ce contrat n'a jamais décrite — et elle contredit (a).**
> L'implémentation rend le numéro **existant** si la clé `(CodeMission, EtiquetteLocale)` est déjà
> au registre. Une réémission voulue recevrait donc l'**ancien** numéro, **silencieusement**.
> Conséquence observée : les numéros du 05/08/2026 ont été saisis **à la main** par le gardien, et
> non alloués par l'outil — le réel a contourné un outil qui ne savait pas faire ce que le métier
> demande.
> **(d) Correctif d'architecte : l'idempotence CHANGE DE CLÉ, elle ne disparaît pas.** Elle cesse
> de porter sur la **prestation** et porte sur le **GESTE** : l'appelant fournit une
> **`CleEmission`** — identifiant unique de la confirmation — et l'outil est idempotent **sur
> elle**. Un **rejeu** (double clic, retry réseau, réponse perdue) rend le **même** numéro ; une
> **nouvelle confirmation** alloue un numéro **neuf**. Les deux exigences tiennent ensemble :
> « une émission = un numéro » **et** « aucun numéro brûlé par un incident technique ». Colonne
> **`CleEmission`** (texte) à créer au registre — **RUNBOOK GARDIEN**. Correctif d'implémentation :
> **PR séparée** sur `outils/mcp-graph/`, préalable **bloquant** de `T-0052`.
> **(e) Ordre — contrôle à ajouter.** Sur une année, `DateEmission` doit être **non décroissante**
> avec `NNNN`. Une violation est une **ANOMALIE SIGNALÉE** — jamais corrigée en silence, **jamais**
> par réécriture d'un numéro déjà alloué.
> **(f) Format hétérogène de la série 2026 — CONSTATÉ, NON corrigé.** `F-2026-001` à `-003` (trois
> chiffres, seed du 28/07) coexistent avec `F-2026-0004` et suivants (quatre chiffres, forme du
> code). **Un numéro émis ne se réécrit jamais** : les trois premiers restent tels quels, tout
> numéro alloué désormais porte **quatre** chiffres. Écart consigné, à expliquer au comptable,
> **jamais à « nettoyer »**.
> **(g) DOMICILE DE LA VÉRITÉ (arbitrage gardien du 06/08).** Le **classeur de saisie déposé fait
> foi** pour la facturation. Le registre « Factures » porte le **NUMÉRO** — la séquence légale — et
> **non** la vérité de ce qui est dû. L'**acte comptable** (avoir, refacturation, retenue de la
> bonne facture) relève du **comptable**, **hors SI**. **Fait mesuré le 06/08** : le registre
> portait deux numéros pour `2026-06-siteflow` et deux pour `2026-07-siteflow`, à 19 800 € (28/07)
> et 16 830 € (05/08) ; la **saisie** dit **16 830 €** (formule `=18,7*900`). Ce n'est **pas** un
> incident : les saisies sont des **sources vivantes**, la dernière version fait foi, et l'écart
> part au comptable.
> **(h) Statuts de l'onglet « Facturation » — TOLÉRANCE DE LECTURE.** Valeurs **réelles** mesurées
> le 06/08 dans `saisie-1-siteflow.xlsx` : `Emise`, `emise`, et **`a emmettre`** (deux « m », sans
> accent) — là où §5.2 dit « à émettre / émise / payée ». Tout consommateur **tolère** casse,
> accents et cette faute de frappe **à la lecture** ; **aucun ne réécrit la saisie** — l'invariant
> « la machine n'écrit jamais la saisie » **prime** sur la propreté du vocabulaire. Un **filtre
> strict serait un défaut** : il rendrait le bandeau 5 **vide** alors que huit lignes existent.
...
| Identité Entra | `IdentiteEntra` | Texte | UPN, renseignée quand elle existe |
| Disponibilité | `Disponibilite` | Date | disponibilité annoncée |
...
- **Saisie (surface équipe, source humaine).** Classeurs `saisie-...xlsx` à la racine de la
  ...
  (`tour-de-controle.md` §1, §4) — **à l'exception BORNÉE de la cascade d'émission de facture**
  (`tour-de-controle.md` v2.3 §1 / §3 bandeau 5) : le statut « émise » s'y écrit par un **geste
  humain confirmé**, **sous l'identité du cliqueur, jamais machine** ; le **domicile exact** de
  cette écriture (saisie sous identité déléguée vs registre « Factures » re-dérivé) est **tranché
  à l'ouverture de `T-0052`**. L'invariant « **la machine n'écrit jamais la saisie** » est
  **inchangé**. Coordonnées tenant : `GRAPH_SAISIE_DRIVE_ID` /
...
- **Rituel des temps (institué le 01/08/2026, S46 — cadence MENSUELLE, arbitrage gardien).** À la
  clôture de chaque mois — **au plus tard le 2e jour ouvré du mois suivant** — chaque collaborateur
  affecté à une mission **déclare ses jours RÉALISÉS du mois écoulé** dans la grille
  `SAISIE_Realise_<millésime>` du classeur de saisie de sa mission : **sur SA ligne** (colonne
  `Ressource` = son **identifiant §5.3** — mail Allia pour un salarié, mail du sous-traitant pour un
  sous-traitant, le **MÊME identifiant** que `T_Ressources`), **à la POSITION du mois** (une cellule
  = le **cumul des jours du mois** ; la colonne `TOTAL` du classeur se recalcule). Édition Excel
  Online, **co-édition tolérée** — le **contenu fait foi** (règle ci-dessus). La déclaration est une
  **SAISIE SOURCE** : la **machine ne l'écrit jamais** (droit `read`, invariant ci-dessous). La
  **dérivation** vers `T_Imputations` (règle de dérivation ci-dessous, skill
  `consolidation-pilotage`) **suit** la déclaration — aujourd'hui **déclenchement manuel du
  gardien** ; les lignes dérivées naissent `StatutValidation` « à valider » (§5.2), **aucune
  validation outillée en v1**. **V1 MANUELLE** : **aucun rappel, aucun contrôle de complétude** —
  l'**orchestration** est **NOMMÉE, non construite** (`T-0046`). **SURFACE FUTURE NOMMÉE**, non
  construite (`T-0046`) : un **écran de déclaration de temps** (fonction des **missions** du
  collaborateur et de son **prévisionnel**), qui écrirait la saisie sous l'**identité DÉLÉGUÉE du
  déclarant** — **jamais l'identité machine**, l'invariant « la machine n'écrit jamais la saisie »
  tient — et exigera une **évolution du contrat `tour-de-controle.md`** (§1/§4 : cockpit
  aujourd'hui en **lecture seule** sur l'économique).
...
### 5.7 Budget entreprise (saisie gardien)

- **Domicile.** Classeur de **saisie humaine** du budget entreprise —
  **`budgetentreprise.xlsx`** (nom réel au tenant, **sans tiret**) : site
  **`AlliaConsulting-Contratsetadministratif`**, bibliothèque « Documents », dossier
  **« 09 - Budget Allia »**. **Adressage par item ID, jamais par chemin** (un chemin se renomme,
  un ID ne bouge pas) — **adresse probante** relevée le 04/08/2026 :
  `driveId = b!44CicXj16kaqbdbelsTnPUundQ8Qe5JKmBk7kAxljRO8Hx-KrTj1QJbs4Mp6eHK7`,
  `itemId = 01BWFCBZFWGJWKUVUL4JC3TIYEJKALAW52`. Les coordonnées **d'exécution** du cockpit
  restent portées par **variables d'environnement** posées par runbook gardien (même régime que
  le référentiel de coûts, §5.5) ; l'adresse ci-dessus est **la trace de la mesure**, pas un
  paramètre de configuration.
- **État mesuré — le classeur est DÉPOSÉ et PEUPLÉ.** Déposé le **03/08/2026** par le gardien,
  **relu sous identité DÉLÉGUÉE le 04/08/2026** (lecture directe du classeur, **pas l'index de
  recherche**) : **2 onglets** — `Lisez-moi` (conventions) et `Budget` (la table). `T_Budget`
  porte **36 lignes, une par mois, de 2026-01 à 2028-12** ; les **premiers mois de 2026 sont
  vides** — la convention du mois vide ci-dessous **joue déjà sur le réel**. `EBITDA_Budget` est
  bien une **formule** (`=IF(COUNT(CA:CS)=0,"",CA − MS − CS)`), jamais saisi. La **part variable**
  est **incluse** dans `MasseSalariale_Budget`, au prorata mensuel (arbitrage gardien du 03/08 ;
  **contrôle de relecture** : janv-2027 → MS **97 987 €**, EBITDA **23 108 €**). Le dossier porte
  aussi `BP allia.xlsx` (modélisation du BP) et `Business plan et tréso.xlsx` (**sensible**).
  Chantier consommateur : **`T-0051`**.
- **Cécité de l'identité machine — attendue, non bloquante.** Le **serveur Workbook (identité
  MACHINE)** ne voit pas ce site : `Sites.Selected` n'y est **pas octroyé**. Ce n'est **pas** une
  absence du classeur — **une absence vue par une identité sans droit n'est pas une absence**
  (même famille que le faux « gabarit absent » de `T-0045`). **Aucun octroi machine n'est
  requis** : le cockpit lit ce classeur sous **identité UTILISATEUR**, et toute vérification se
  fait sous **identité déléguée**. Corollaire opérationnel : **ne jamais conclure l'absence de ce
  classeur depuis le serveur Workbook**, ni depuis un index de recherche.
- **Écart de pointeur à refermer (consigné, non traité ici).** `tour-de-controle.md` **v2.3
  (promu)** écrit `budget-entreprise.xlsx` — **avec** tiret. Le **nom réel au tenant fait foi** :
  `budgetentreprise.xlsx`. Correction de **pointeur** à porter à la prochaine ouverture de
  `tour-de-controle.md` (**micro-PR dédiée, une ligne**) ; ce contrat ne modifie pas
  `tour-de-controle.md`.
- **Régime d'accès (arbitrage gardien du 03/08/2026).** Le **SITE** est à **audience restreinte**
  parce qu'il héberge aussi `Business plan et tréso.xlsx` — les deux sources **ne sont pas
  séparées** dans l'architecture documentaire. Mais les **données du budget ne sont PAS
  sensibles** : elles s'affichent à **toute la firme** au cockpit, sans mesure de protection à la
  lecture. Conséquence, **sans élévation de droits** : l'accès **LECTURE du seul dossier
  « 09 - Budget Allia »** est ouvert à la firme par **geste gardien** (héritage rompu, octroi
  lecture — **runbook tenant**, `organisation.md` §5) ; **le reste du site, BP & Tréso compris,
  demeure restreint**. Le cockpit lit `T_Budget` **sous l'identité de l'utilisateur connecté** :
  c'est l'**octroi de lecture du dossier** qui rend la colonne Budget lisible par toute la firme,
  **jamais un contournement** ni un compte de service.
- **Schéma — table nommée `T_Budget`** (une ligne par mois) :

| Colonne | Type | Notes |
|---|---|---|
| `Mois` | Date | **1er du mois** (même convention que `T_Structure`, §5.3) |
| `CA_Budget` | Nombre | € **HT** (convention §5.4) |
| `MasseSalariale_Budget` | Nombre | € HT — **part variable incluse**, au **prorata mensuel** |
| `CoutsStructure_Budget` | Nombre | € HT |
| `EBITDA_Budget` | Nombre | **formule du classeur — JAMAIS saisi** |

- **Convention du mois vide.** Un mois **sans budget** est une ligne aux **montants VIDES** —
  **jamais un zéro inventé** (même règle que l'affichage « · » du cockpit,
  `tour-de-controle.md` §3).
- **La machine ne l'écrit JAMAIS.** Le budget est une **source humaine** : l'invariant « la
  machine n'écrit jamais la saisie » (§5.6) s'y étend par construction. **État du droit au
  04/08/2026** : l'identité machine n'a **aucun octroi** sur ce site (`Sites.Selected` non
  accordé) — l'invariant est donc garanti **au niveau du droit**, et même **plus strictement**
  que pour la couche de saisie (qui, elle, porte un `read`). Si un octroi machine devait un jour
  être posé, il ne pourrait être qu'un **`read`** — **jamais d'écriture**, et **jamais de scope
  `Files.*`** (même règle qu'en §5.6 : cela outrepasserait `Sites.Selected`).
- **Consommation.** Le cockpit lit `T_Budget` pour la **colonne Budget du bandeau 4**
  (`tour-de-controle.md` v2.3 §3), **filtrée sur l'année affichée** (année en cours, bouton
  « Année N+1 »). Le **Réalisé** du même bandeau reste **dérivé** des gabarits actifs et du
  référentiel de coûts (§5.4) — **inchangé** : budget et réalisé ne partagent aucune source.

- **RÉGIME D'ACCÈS RÉVISÉ — arbitrage gardien du 06/08/2026 (S50).** Bloc **AJOUTÉ** ; **aucun
  tiret ci-dessus n'est réécrit** — les énoncés démentis sont corrigés **par renvoi d'ici**
  (forme `backlog/plan.md` §18).
  **(a) Décision.** Le dossier « 09 - Budget Allia » **n'est pas** ouvert en lecture à la firme.
  L'octroi décrit au tiret « Régime d'accès » est **ABROGÉ** : il n'a jamais été joué et ne le
  sera pas. Le classeur `budgetentreprise.xlsx` demeure **réservé**, au niveau du fichier.
  **(b) Fait mesuré le 06/08/2026** (identité déléguée, `_api …/web/sitegroups`, lecture
  d'objet) : le site ne donne accès qu'à **deux personnes** — groupe **Membres** (Modification)
  = 2 utilisateurs nommés, **Propriétaires** (Contrôle total) = les 2 mêmes, **Visiteurs**
  (Lecture) = **VIDE** ; ce sont des attributions **directes**, pas un groupe imbriqué. Le site
  était donc **déjà** restreint à deux personnes avant tout geste. L'héritage du dossier a été
  **rompu par le gardien le 06/08** (`HasUniqueRoleAssignments` = `true`, mesuré) ; les
  attributions **recopiées** par SharePoint à la rupture ont été **laissées telles quelles**, la
  restriction étant déjà tenue par la composition des groupes.
  **(c) Séparation documentaire — JOUÉE le 06/08/2026.** `BP allia.xlsx` et `Business plan et
  tréso.xlsx` ont été déplacés vers le dossier frère **« 10 - Business plan »** (même
  bibliothèque, même `driveId`, `itemId` **préservés** — un déplacement intra-drive ne change pas
  l'identifiant). Preuve après geste : « 09 - Budget Allia » ne porte plus qu'un fichier,
  `budgetentreprise.xlsx`, dont l'**adresse probante gravée ci-dessus est inchangée** ; le dossier
  « 10 » **hérite** du site restreint (`HasUniqueRoleAssignments` = `false`). La contradiction
  interne de cette section est ainsi refermée **par le geste**, pas par la prose.
  **(d) L'état du droit du tiret « La machine ne l'écrit JAMAIS » est FAUX — et l'était à sa
  rédaction.** `architecture.md` (registre des octrois) porte depuis `T-0031` :
  identité managée `id-allia-mcp-graph`, site **Contrats-et-administratif**, `Sites.Selected`
  **`role=write`**. Corroboré par l'usage, non par la prose : les gabarits ERP de
  « 06 - Gabarit ERP » sont **créés et régénérés par la machine sur ce site** depuis le 17/07, et
  `inscrire_cout_structure` y **écrit** `referentiel-structure.xlsx` (« 07 - Coût de Structure »)
  depuis le 01/08 — une identité qui écrit sur un site le voit. **Conséquence à ne pas
  euphémiser** : l'invariant « la machine n'écrit jamais le budget » est garanti par le **CODE**
  (primitives à **cible figée**, aucune ne pointant sur ce classeur), **PAS par le droit** ; la
  phrase « plus strictement que pour la couche de saisie » est inexacte — c'est l'inverse, la
  saisie porte un `read` là où ce site porte un `write`. **Cause de l'erreur** : aucune primitive
  ne pointant sur ce classeur, le serveur ne pouvait pas le lire — on a conclu d'une **absence
  d'OUTIL** à une **absence de DROIT**. *Le moyen n'est pas l'effet.*
  **(e) Régime cible — NOMMÉ, NON CONSTRUIT.** La donnée budget reste affichée à **toute la
  firme** au cockpit **sans élévation de droits**, par **PROJECTION DÉRIVÉE** sur le patron des
  gabarits ERP (§5.1) : le serveur lit `T_Budget` **sous identité machine** et écrit une **Liste
  « Budget »** sur le site **AlliaConsuling** ; le **cockpit lit cette liste** sous l'identité de
  l'utilisateur connecté, comme il lit déjà « CRM » et « Missions ». Le classeur demeure la
  **SOURCE**, réservée et **jamais écrite par la machine**. Aucun octroi nouveau n'est requis
  (`write` sur les deux sites, déjà au registre). **Schéma de la liste, noms internes (§2 ter),
  entrée §2 bis et cran de l'outil de projection : NON gravés ici** — à l'ouverture de `T-0051`,
  après la mesure.
  **(f) Point NON ACQUIS, à éprouver avant la première ligne de code.** Les **permissions
  uniques** posées le 06/08 sur le dossier peuvent **bloquer un jeton app-only** malgré
  `Sites.Selected`. **À MESURER** côté serveur (lecture réelle du classeur), **jamais à déduire** :
  c'est la **première épreuve** de `T-0051`. Si elle est rouge, la projection exige d'ajouter
  l'application aux autorisations du dossier — **runbook gardien**, à instruire à ce moment-là.
  **(g) Renvoi.** `tour-de-controle.md` **v2.5 §3 bandeau 4** écrit que le cockpit lit le
  **classeur** ; avec (e) il lira la **liste projetée**. Corrigé **par renvoi d'ici**, **non
  réécrit là-bas** — à porter à la **micro-PR déjà due** sur ce contrat (écart de pointeur
  `budget-entreprise.xlsx`, tiret « Écart de pointeur à refermer » ci-dessus), jamais dans cette
  PR-ci.
