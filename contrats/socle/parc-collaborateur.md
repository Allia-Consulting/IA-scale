...
> **Version** : 1.5 — *candidat*. **Statut** : contrat socle — fait foi (le gardien bascule *candidat* → *promu* au merge).
> **Changelog** :
> - v1.5 (enrichissement candidat) — 28 juillet 2026 : §5 — **revirement de la politique de privilège du compte utilisateur** : chaque collaborateur devient **administrateur local** de son Mac (membre du groupe admin macOS), promu par un **script shell Intune** assigné au **groupe des Macs**, et **installe ses applications en autonomie, sans validation ni saisie de mot de passe administrateur tiers**. Abroge la politique antérieure (compte **Standard**, installation médiée) sur laquelle reposaient plusieurs mécanismes du §4 (l'invite de mot de passe administrateur qui motivait la sous-section « Claude Desktop » devient sans objet ; le choix `~/Applications` **reste** le standard sur ses propres mérites — renoncement borné au contrôle de version de la flotte —, il n'est **pas** rouvert par ce revirement). **Compensations maintenues, non négociables** : FileVault, enrôlement verrouillé ADE (profil MDM non amovible), compte administrateur **LAPS Intune distinct** conservé pour le support/break-glass, conformité Intune inchangée. Rollback en **une action** (retrait de l'assignation du script + script inverse `dseditgroup -o edit -d`). Exécution = runbook `T-0044`. **Décision du gardien du 28 juillet 2026** (revirement explicite de la politique de parc).
> - v1.4 (enrichissement candidat) — 15 juillet 2026 : §4 — politique d'**emplacement et de mise à jour de Claude Desktop**, jusqu'ici absente du contrat : déploiement en `~/Applications` (dossier utilisateur), **auto-update natif conservé**, **aucune clé gérée** dans `com.anthropic.claudefordesktop`. Tranche une décision restée ouverte depuis le 13 juin 2026 (symptôme : invite de mot de passe administrateur au clic « relancer pour mettre à jour » sur un compte Standard, l'updater ne pouvant réécrire le bundle en `/Applications`). Renoncement **explicite et borné à Claude Desktop** au contrôle de version de la flotte. Exécution = runbook `T-0036`. Décision du gardien du 15 juillet 2026.
...
> **Rattachement** : capacité *Socle d'exploitation & gouvernance / parc & gestion de poste* ; chantiers `backlog/chantiers/T-0006.yaml` (config effective, runbook), `backlog/chantiers/T-0008.yaml` (réconciliateur politique→Intune, Phase 3/4), `backlog/chantiers/T-0036.yaml` (reconditionnement emplacement Claude Desktop + migration parc, runbook) et `backlog/chantiers/T-0044.yaml` (application de la politique administrateur local sur le tenant, runbook Intune).
...
- **Socle** (poussé par défaut via Intune) : suite Microsoft 365 (Word, Excel, PowerPoint, Outlook, Teams, OneDrive) ; Claude desktop (emplacement `~/Applications` + auto-update natif — voir sous-section dédiée §4) ; Claude Code CLI (l'authentification OAuth des serveurs MCP `.mcp.json` projet se fait au CLI — fait durci T-0010) ; Google Chrome (navigateur géré) ; git et gh (la connexion Git vise le dépôt commun de la firme — org GitHub Allia-Consulting ; le geste d'authentification `gh auth login` reste celui de la personne, sous son identité).
...
### Claude Desktop — emplacement d'installation et mise à jour

- **Emplacement d'installation** : Claude Desktop est déployé dans le **dossier utilisateur `~/Applications`**, **jamais** dans `/Applications`. Motif (cause racine, documentée par Anthropic) : l'updater intégré réécrit le **bundle `Claude.app` lui-même** ; en `/Applications` (dossier système), un compte **Standard** ne peut pas écrire le bundle → élévation → **invite de mot de passe administrateur** à chaque mise à jour. En `~/Applications`, le bundle appartient à l'utilisateur : la mise à jour s'applique **sans privilège administrateur**.
- **Mise à jour** : l'**auto-update natif** de l'application est **conservé** — **aucune clé gérée `disableAutoUpdates`** n'est posée dans le domaine `com.anthropic.claudefordesktop`. Objectif : mise à jour par un compte Standard **sans invite de mot de passe et sans geste récurrent de l'administrateur SI**.
- **Renoncement assumé et borné** : conserver l'auto-update natif, c'est **renoncer au contrôle de version de la flotte** pour Claude Desktop — Anthropic pilote le rythme et le contenu des mises à jour. Ce renoncement est **explicite, assumé et strictement borné à Claude Desktop** ; il ne s'étend à **aucun** autre composant du socle. Il est cohérent avec la posture correctifs (RGPD art. 32 « intégrité/disponibilité » ; CyberVadis « gestion des vulnérabilités ») : une flotte qui suit le canal officiel signé Anthropic vaut mieux qu'une flotte fragmentée entre versions.
- **Aucune clé gérée** : la politique retenue ne pose **aucune** clé du domaine `com.anthropic.claudefordesktop`. Conséquence voulue : **aucun risque** de basculer **Cowork** ou l'inférence tierce en « géré par l'organisation » (comportement historique « tout-ou-rien » d'une source gérée). Toute pose ultérieure d'une clé gérée (ex. `forceLoginOrgUUID`) est un **candidat distinct**, conditionné à une preuve sur poste pilote que Cowork reste éditable.
- **Mesures compensatoires** (maîtrise sans geste récurrent) : (1) **télémétrie de version** via Intune (lecture `~/Applications/Claude.app/Contents/Info.plist` → `CFBundleShortVersionString`) ; (2) **seuil de version minimale** acceptable + **alerte de dérive** (poste bloqué sous le seuil ou updater cassé).
- **Réversibilité** : réversible par re-push Intune du `.pkg` `/Applications` ; coût = un cycle de redéploiement + nettoyage des `~/Applications`. Aucune perte de donnée, aucun secret.
- **Exécution = runbook humain** : le reconditionnement Intune, la détection et le **nettoyage des doubles installations** (`/Applications` **et** `~/Applications` sur un même poste), la validation sur poste pilote et la généralisation relèvent de `backlog/chantiers/T-0036.yaml` — jamais d'un agent (frontière §7). T-0006 (soldé) n'est pas rouvert.
...
### Privilège du compte utilisateur — administrateur local (revirement du 28 juillet 2026)

Sur **décision du gardien du 28 juillet 2026** (revirement explicite de la politique antérieure), la posture de privilège du compte utilisateur passe de « utilisateur **standard**, installation médiée » à **administrateur local**. La politique « compte Standard » est **abrogée**.

- **Chaque collaborateur est administrateur local de son Mac** : membre du groupe administrateurs macOS. La promotion est faite **par un script shell Intune** assigné au **groupe des Macs** (groupe d'appareils) — jamais un ajout manuel poste par poste. C'est un dérivé projeté d'une décision promue ici (doctrine §8), pas une configuration saisie à la main.
- **Installation d'applications en autonomie** : le collaborateur installe ses applications **lui-même, sans validation** et **sans saisie de mot de passe administrateur tiers**. L'installation médiée (poussée sous compte de service, `.pkg` élevé par un tiers) n'est plus le canal par défaut.
- **Compensations maintenues — non négociables** (le passage en admin local ne relâche aucune d'elles) :
  - **FileVault** obligatoire, clé de récupération séquestrée dans Intune (inchangé, cf. ci-dessus) ;
  - **enrôlement verrouillé ADE** : profil MDM **non amovible** — un administrateur local ne peut ni désenrôler le poste ni retirer la gestion ;
  - **compte administrateur LAPS Intune distinct** conservé, à mot de passe géré/roté par Intune, réservé au **support et au break-glass** — indépendant du compte nominatif du collaborateur ;
  - **conformité Intune inchangée** : les stratégies de conformité et l'accès conditionnel `ca-parc-collaborateur-conforme` continuent de conditionner l'accès M365 à l'état « appareil conforme ».
- **Réversibilité — une action** : retirer l'assignation du script de promotion **et** appliquer le script inverse `dseditgroup -o edit -d` (retrait du groupe admin). Aucune perte de donnée, aucun secret. Le poste redevient Standard.
- **Articulation avec le §4** : ce revirement rend **sans objet** le symptôme d'invite de mot de passe administrateur qui motivait des passages de la sous-section « Claude Desktop » ; il **ne rouvre pas** la décision `~/Applications` + auto-update natif, qui tient sur ses propres mérites (renoncement borné au contrôle de version de la flotte) et reste le standard.
