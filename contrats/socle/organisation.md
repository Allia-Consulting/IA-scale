...
> **Version** : 1.11 — *promu*. Troisième contrat socle, aux côtés de `/doctrine/doctrine.md` et `/backlog/plan.md`.
...
## 3 ter. Les collaborateurs (état réel)

*Domicile canonique du statut de collaborateur.* Là où la table §3 enregistre les **animateurs** de périmètre (une délégation), cette table enregistre le **statut de collaborateur** lui-même — la décision promue « X est collaborateur d'Allia », source dont les **groupes socle** (§5 : licence, parc, SharePoint socle) sont la projection. On nomme par **identifiant Entra**, sans dupliquer aucun dossier RH (§1, §7).

| Collaborateur (identité Entra) | Statut | Depuis |
|---|---|---|
| adrien.raque@allia-consulting.com (fondateur — gardien & associé) | collaborateur | fondation |
| sarah.shaiek@allia-consulting.com | collaboratrice (aussi animatrice Communication & marque, §3) | 15 juin 2026 |
| Guillaume.Dan@allia-consulting.com | collaborateur | 28 juin 2026 |

*Un collaborateur listé ici appartient, du seul fait de son statut, aux **groupes socle** (§5) ; l'onboarding (capacité chapeau, §2 ; skill `skills/onboarding/SKILL.md`) en est la projection réconciliée. Ajouter une ligne ici est la **décision promue** que l'onboarding consomme — jamais l'inverse.*
...
### §4.1 Délégations de promotion (état réel — registre)

Le registre annoncé ci-dessus ouvre avec sa première entrée, bornée et révocable en un repointage :

| Périmètre de promotion délégué | Promoteur délégué | Bornes | Depuis |
|---|---|---|---|
| `/contrats/local/communication/` (contrat local uniquement) | Sarah Shaiek (sarah.shaiek@allia-consulting.com) | Socle et doctrine EXCLUS. Promotion locale en régime nominal ET suppléance en absence du gardien, sur ce seul périmètre. | 3 juillet 2026 |
| `/skills/newsletter-hebdo/` et `/evals/skills/newsletter-hebdo/` (skill et golden set uniquement) | Sarah Shaiek (sarah.shaiek@allia-consulting.com) | Socle et doctrine EXCLUS. Promotion du skill newsletter-hebdo et de son golden set, en régime nominal et en suppléance, sur ces seuls chemins. | 13 juillet 2026 |

**Mise en œuvre machine** (la règle vit ici ; la machine l'outille, jamais l'inverse) :
- CODEOWNERS : `/contrats/local/communication/ @SarahSK-75 @Alliaconsulting` — en place.
- CODEOWNERS : `/skills/newsletter-hebdo/ @SarahSK-75 @Alliaconsulting` et `/evals/skills/newsletter-hebdo/ @SarahSK-75 @Alliaconsulting` — ajoutés (v1.11, préalable T-0011).
- Ruleset `main-protection` : PR obligatoire, 1 approbation, revue Code Owners requise, checks requis « Avis d'impact » (agent-gardien) et « Harnais skills » (evals T-0020-a), bypass réservé au rôle Repository admin (flux solo du gardien — un auteur ne peut pas s'auto-approuver).
- Congé du gardien : ajout **temporaire** de Sarah aux bypass_actors, retrait au retour — les deux gestes sont journalisés. Cible propre : T-0020-c (auto-approbation du faible risque contre politique par l'agent-gardien) rendra ce bypass temporaire inutile.
...
- le réconciliateur est au **moindre privilège**, atteint par **identité managée zéro-secret** : un rôle Entra « Administrateur de groupes » est assigné au service principal de l'identité managée `id-allia-mcp-graph`, **scopé à l'Administrative Unit `au-perimetres-droits`** — qui ne contient que les groupes de périmètre, rien d'autre — **plus** une liste blanche figée côté serveur (`GRAPH_GROUPES_PERIMETRE_AUTORISES`) en défense en profondeur ; tout changement tracé. *(L'app registration + secret reste une alternative possible mais non retenue : l'identité managée sans secret est cohérente avec la doctrine zéro-token-store du MCP, T-0015.)* ;
...
**Runbook humain (ni Claude ni agent) :** assigner le rôle Entra « Administrateur de groupes » — **scopé à l'Administrative Unit `au-perimetres-droits`** — au service principal de l'identité managée, et figer la liste blanche côté serveur. **Aucun secret n'est créé, distribué ni stocké** (zéro-token-store). Ni Claude ni un agent ne configurent de permissions ni ne posent ce privilège (plan §2).
...
4. Les droits sont un dérivé réconcilié de la décision promue (groupes Entra), jamais saisis ; la porte humaine reste à la promotion, le réconciliateur au moindre privilège. La mise en place du privilège reste un runbook humain — par **identité managée + rôle Entra scopé à une Administrative Unit de périmètre, sans secret** (et non app registration + secret).
