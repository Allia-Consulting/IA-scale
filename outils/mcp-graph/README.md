...
Un serveur [Model Context Protocol](https://modelcontextprotocol.io), exposé en **HTTP streamable**, qui donne à un agent **seize** opérations sur Microsoft Graph, et seize seulement :
...
| `allouer_code_mission(opportunite_id)` | **écriture source (bornée)** | `GET .../lists/{CRM}/items/{id}?$expand=fields` puis `PATCH .../items/{id}/fields` avec **If-Match** — alloue **atomiquement** le `CodeMission` (`max+1`) d'une **opportunité GAGNÉE** et le **réécrit sur elle** ; cible **FIGÉE** Liste « CRM » (`GRAPH_CRM_LIST_ID`), **colonne `CodeMission` uniquement** ; préconditions **fail-closed** (`Etape=Gagnée` ET `CodeMission` vide) ; ETag/If-Match + **post-vérification anti-course** bornées ; cran **notifie** (`T-0038`). |
| `allouer_num_facture(code_mission, etiquette_locale, mois_ca, montant_ht, echeance)` | **écriture source (bornée)** | `GET .../lists/{FACTURES}/items?$expand=fields` (scan) puis `POST .../items` — alloue le **NumFacture légal `F-AAAA-NNNN`** (`max de l'année + 1`, `0001` si aucun) d'une échéance à **l'émission** et l'inscrit au **registre « Factures »** ; cible **FIGÉE** (`GRAPH_FACTURES_LIST_ID`, **même site que « CRM »**) ; **IDEMPOTENT** par clé (`CodeMission`, `EtiquetteLocale`) — re-ingestion ne réalloue jamais ; `Statut=« émise »` + `DateEmission` posés **côté serveur** ; **post-vérification anti-course** bornée (tie-break déterministe par `item_id` : renumérotage `PATCH` If-Match du perdant, jamais de trou, jamais de recyclage) ; **aucune suppression** ; cran **validé** (`T-0030`). |
...
5. **Preuve EN SESSION (sanity chaude)** — chaque table est relue (`/rows`, session chaude) et **doit être VIERGE** (cf. prédicat ci-dessous) ; **nécessaire mais non suffisant** (cf. étape 8) ;
...
8. **Preuve FROIDE, SANS session (le chemin EXACT du cockpit)** — chaque table est relue par `/columns` **sans** `workbook-session-id` (comme `workbook-graph.ts` de la tour de contrôle) et **doit** s'ouvrir à froid (`200`), porter ses en-têtes §5.2 et être **VIERGE**. C'est **cette** preuve, et non l'étape 5, qui atteste l'ouvrabilité dont le cockpit a besoin ; un échec froid (`403`/`404`/`500` ou table non vierge) déclenche le rollback — **plus jamais de faux-vert**.

**Prédicat « vierge » (recalibré, `T-0035` reprise n°5).** Une table fraîchement fabriquée est **vierge** si elle porte ses en-têtes §5.2 exacts **et aucune ligne de corps PLEINE** (portant une valeur) — les lignes **entièrement vides** sont **tolérées**. Fait mesuré le 17/07 : une table Excel vide porte une **ligne d'insertion** standard (fabrication service `ref=A1:X2`/insertRow=1 ; classeur Excel **authentique** `ref=A1:D3`, deux lignes vides), que le peuplement résorbe — exiger un `count:0` **strict** avortait l'instanciation à tort (faux-négatif de l'épreuve tenant du soir). La voie « souche binaire copiée » (`/copy`) a été **évaluée puis écartée** (Excel écrit lui aussi des lignes vides — elle échouerait à l'identique) : la **fabrication service est conservée**.

**Rollback borné ET VÉRIFIÉ (`T-0035` reprise n°5).** Toute erreur survenant **après** la création (étape 1) supprime le **seul** item créé par l'appel courant — jamais un autre — **puis relit l'item** (`GET` → `404` attendu). On n'annonce **jamais** une suppression non prouvée (l'ancien rollback « best effort » avait menti — item réputé supprimé resté en racine, épreuve tenant du 17/07) : si l'item est encore présent, le statut le dit honnêtement (« rollback incomplet, item à retirer par le gardien »). Le schéma des 3 tables **dérive de** `modele-donnees.md` §5.2, qui **fait foi** (le binaire `gabarit-pilotage-mission.xlsx` reste retiré du rôle de souche, conservé au canon comme trace historique). Retour : `{code_mission, nom_gabarit, item_id, tables: {T_Affectations: {lignes_vides: N}, …}}` — chaque table prouvée **vierge à froid** (étape 8), le compte des lignes d'insertion vides tolérées.

**Correctif 0.19.2 (épreuve 3g du 31/07, journal `docs/epreuves/2026-07-31-t0030-epreuve-3g-stop-schema.md`).** La projection `TABLES_GABARIT` de `T_Echeancier` était restée à **7 en-têtes** alors que `modele-donnees.md` §5.2 **v1.25** en fait foi avec **8** : `EtiquetteLocale` (clé de réconciliation `(CodeMission, EtiquetteLocale)` avec la saisie, §5.6) manquait — trou de la chaîne PR #268 (contrat) → #272 (skill, « aucun code serveur »), la projection serveur n'ayant été portée par aucune des deux. `T_Echeancier` re-projeté à l'ordre exact du contrat (`NumFacture, CodeMission, EtiquetteLocale, MoisCA, MontantHT, Echeance, Statut, LienFacture`) ; `T_Affectations`/`T_Imputations` vérifiés conformes, inchangés. Un **test anti-divergence** compare désormais les en-têtes des 3 tables à la liste littérale de §5.2 (casse la CI si la projection re-diverge).

**`workbook_archiver_gabarit` — DÉPLACEMENT, pas copie (`T-0035` reprise n°4, serveur 0.15.0).** L'archivage **DÉPLACE** (`PATCH` de l'item : nouveau `parentReference` « 00 - Old » + nom horodaté) le gabarit courant, au lieu de le **copier** (`POST /copy`). La copie laissait l'original à la racine : le nom `gabarit-<code>.xlsx` n'était **jamais libéré**, si bien que la **régénération** d'un gabarit existant (archiver → ré-instancier → repeupler, orchestrée par le skill `consolidation-pilotage`) butait sur une **collision** contre le fail-closed de `workbook_instancier_gabarit` (épreuve tenant du 17/07). Le déplacement **libère le nom** et rend la ré-instanciation possible **sans affaiblir le fail-closed**. Ce n'est **pas** une primitive de déplacement/suppression générique : source (`gabarit-<code_mission>.xlsx` de « 06 - Gabarit ERP ») et destination (« 00 - Old » figé) sont **résolues côté serveur** ; l'appelant ne fournit que `code_mission`. Le *move* au sein d'un même drive est **synchrone** (`200`) — plus de `202`/`Location` à poller. **Réversible** (version conservée intacte dans « 00 - Old ») ; le rétablissement inverse reste un geste **gardien** (aucune primitive ne l'expose).

Cran des écritures gabarit : **auto** (`table-des-crans.yaml` : `reconcilier_gabarit_pilotage` / `instancier_gabarit_pilotage`) — interne, local, réversible. La **mise en service** (domicile « 06 - Gabarit ERP » / « 00 - Old », pose des variables) est un **runbook gardien** (`T-0031`) ; le **build + déploiement** de la v2 (image serveur **0.13.0**, `T-0033`), puis de la reprise **ouvrabilité à froid** (image serveur **0.14.0**, `T-0035` — ré-émission + preuve froide), puis de l'**archivage par déplacement** (image serveur **0.15.0**, `T-0035` reprise n°4 — libération du nom pour la régénération machine), puis du **prédicat « vierge » recalibré + rollback vérifié** (image serveur **0.16.0**, `T-0035` reprise n°5 — lignes de corps vides tolérées, la ligne d'insertion Excel ; retour `lignes_vides`), et l'**épreuve tenant** (régénérer un gabarit existant : archiver-déplacer → ré-instancier vierge froid → repeupler) restent des gestes gardien après merge.

## Le garde-fou structurel (octies) : l'allocateur ne touche que la colonne `CodeMission` de la Liste « CRM »

`allouer_code_mission(opportunite_id)` (`T-0038`) **ferme la couture opportunité → mission** : à la bascule « Gagnée », l'entier `CodeMission` (identifiant stable de la mission, `modele-donnees.md` §2 bis) est **alloué et réécrit sur l'opportunité**. Jusqu'ici cette réécriture était un **geste gardien tracé** (promotion vers la source CRM) ; l'outil l'automatise, mais reste **borné par construction** :

- **Cible FIGÉE côté serveur** — comme `create_list_item` fige sa liste, l'allocateur n'accepte **aucun** identifiant de liste : la Liste « CRM » est résolue par `GRAPH_CRM_LIST_ID` (`_config_crm()`), et il n'y touche **que** la colonne `CodeMission`. Écrire une autre liste, ou une autre colonne de CRM, est *structurellement* impossible.
- **Préconditions FAIL-CLOSED, avant toute écriture** — l'item doit exister, être à l'étape `Gagnée` **et** avoir un `CodeMission` **vide** ; sinon refus explicite, **rien** n'est écrit. C'est le garde-fou contre la **réattribution** (§2 bis : un code n'est *jamais* réattribué).
- **Allocation ATOMIQUE** — `code = max(CodeMission existants) + 1`. À chaque tentative, l'item est relu pour un **ETag frais**, puis le `PATCH .../fields` porte l'en-tête **`If-Match: <etag>`** : un `409`/`412` (ETag périmé, deux allocations concurrentes) fait **relire → recalculer → réessayer** (borné à 3, puis échec explicite). Après l'écriture, une **post-vérification anti-course** re-scanne la liste : si le code apparaît sur plus d'un item, cet item est **réalloué** (nouveau `max+1`). `Retry-After` est honoré sur `429`/`503`.

Cran : **notifie** (`table-des-crans.yaml` : `allouer_code_mission`) — écriture source réversible et bornée, mais qui ferme une couture firme-visible : l'agent agit, le gardien est informé. Consommé en bout du skill `cadrage-mission` (v1.4), qui appelle ensuite `creer_espace_mission` (auto) — **plus aucun geste gardien dans la chaîne**. La **mise en service** (pose de `GRAPH_CRM_LIST_ID` sur le service, build + déploiement de l'image) et l'**épreuve tenant** (une bascule « Gagnée » produit mission + espace sans geste gardien) restent des gestes gardien après merge.

## Le garde-fou structurel (nonies) : l'allocateur de NumFacture n'écrit qu'au registre « Factures », à l'émission

`allouer_num_facture(code_mission, etiquette_locale, mois_ca, montant_ht, echeance)` (`T-0030`, **image serveur 0.19.0**) est la **source de vérité du NumFacture** légal (`modele-donnees.md` §2 bis). Miroir gouverné de `allouer_code_mission`, il reste **borné par construction** :

- **Cible FIGÉE côté serveur** — comme `create_list_item` fige sa liste, l'allocateur n'accepte **aucun** identifiant de liste : le registre « Factures » est résolu par `GRAPH_FACTURES_LIST_ID` (`_config_factures()`, **même site que « CRM »**), et c'est la **seule** liste écrite. L'appelant ne choisit jamais la cible.
- **IDEMPOTENCE par clé `(CodeMission, EtiquetteLocale)`** — si le registre porte déjà cette clé, l'outil **REND** son `NumFacture` existant **sans créer ni réallouer** (`T-0030` : une re-ingestion ne réalloue jamais un numéro attribué).
- **Allocation à l'ÉMISSION** — `NNNN = max des NNNN de l'ANNÉE + 1` (`0001` si aucun) ; l'élément est créé avec `Statut = « émise »` et `DateEmission` = date du jour, **posés côté serveur** (jamais par l'appelant). Une échéance annulée *avant* émission ne consomme pas l'outil, donc **ne brûle aucun numéro** : chronologie légale FR **sans trou**.
- **Post-vérification ANTI-COURSE, bornée** — après la création, l'année est re-scannée. Tie-break **déterministe** (ordre total sur `item_id`) : sur un doublon de `NNNN`, l'item retenu **garde** le numéro, l'autre (le nôtre, si perdant) est **renuméroté** vers un nouveau `max+1` par `PATCH` **If-Match** — jamais de trou, jamais de recyclage ; ce tie-break garantit la **convergence** (pas de ping-pong). Un doublon de **clé** créé concurremment, si le nôtre n'est pas canonique, est **fail-closed explicite** : l'outil **ne supprime jamais** (aucune primitive), il signale l'élément en trop pour réconciliation gardien. `Retry-After` honoré sur `429`/`503`.
- **Aucune suppression, aucun recyclage** — un numéro alloué est définitif (`supprimer_definitivement` reste proscrit).

**Correctif 0.19.1 (incident du 31/07, journal `docs/epreuves/2026-07-31-t0030-incident-idempotence-0190.md`)** — deux fidélités au réel manquaient au scan du registre : (1) la **lecture** de séquence accepte désormais `F-AAAA-NNN` sur **3 ou 4 chiffres** (`\d{3,4}`) — les **seeds réels** (numéros des PDF déjà émis, sur 3 chiffres) comptent dans le `max` ; sans cela la séquence repartait à `0001`. L'**écriture** reste garantie sur **4 chiffres** (`:04d`). (2) le parse de `CodeMission` tolère la **sérialisation Graph d'une colonne Number** (double intégral, `1` → `1.0`) — sans cela la clé d'idempotence ne matchait jamais et un doublon était écrit. Contrat `modele-donnees.md` §2 bis **inchangé** (seule la lecture de l'existant est assouplie).

Cran : **validé** (`table-des-crans.yaml` : `allouer_num_facture`) — l'allocation grave une **séquence légale irréversible** et la facture **sort de la firme** : porte humaine avant l'action (dé-escalade vers `notifie` envisageable après historique, décision gardien tracée). La **mise en service** (pose de `GRAPH_FACTURES_LIST_ID` sur le service — id de la liste créée par le gardien —, build + déploiement de l'image **0.19.0**) et l'**épreuve tenant** restent des gestes gardien après merge.

## Le garde-fou structurel (decies) : l'inscription du coût de structure n'écrit qu'à `T_Structure`, sur validation d'une ligne candidate

`inscrire_cout_structure(mois, montant, proposition_id)` (`T-0032`, **image serveur 0.20.0**) inscrit le **coût de structure RÉEL** d'un mois dans la table `T_Structure` du référentiel de coûts — c'est la brique serveur du modèle **« coûts standards + contrôle mensuel d'écart »** (`modele-donnees.md` §5.3/§5.4, `table-des-crans.yaml` v1.15). Le réel de fonctionnement contrôlé est une **ligne AGRÉGÉE par mois** (`PosteCout = « fonctionnement-reel »`, `Montant = Σ` des factures fournisseur du mois) ; elle n'entre que **sur validation gardien** d'une **ligne candidate** déposée en Zone-de-proposition. Miroir gouverné de `allouer_num_facture`, l'outil est **borné par construction** :

- **Cible FIGÉE côté serveur** — l'appelant ne fournit **aucun** `drive_id` / `item_id` / nom de table / poste. Le classeur `referentiel-structure.xlsx` est résolu par `GRAPH_REF_STRUCTURE_DRIVE_ID` + `GRAPH_REF_STRUCTURE_ITEM_ID` (`_config_ref_structure()`), la table est `T_Structure` **en dur**, et le `PosteCout` est figé à `« fonctionnement-reel »`. Écrire `T_Parametres`, `T_Ressources` ou un autre classeur est *structurellement* impossible.
- **Préconditions FAIL-CLOSED, avant tout réseau** — variables d'environnement présentes (sinon `ConfigManquante`, **aucun fallback**) ; `mois` = date ISO **au 1er du mois** (une ligne par mois) ; `montant` **> 0** ; `proposition_id` (id de la ligne candidate validée) **obligatoire** — sinon `ValueError`, **rien** n'est écrit.
- **IDEMPOTENCE (Mois, `fonctionnement-reel`)** — le référentiel est **lu d'abord** ; s'il porte déjà une ligne pour ce mois, l'outil **REFUSE** explicitement (`RuntimeError`, la ligne existante citée) : **jamais d'écrasement, jamais de doublon**. La *révision* d'un mois déjà inscrit passe par la **boucle de promotion** (§5.3), pas par une réinscription.
- **Garde ANTI-DIVERGENCE à l'exécution** — les en-têtes servis par `T_Structure` (`/columns`) sont confrontés à la projection contractuelle `ENTETES_T_STRUCTURE` (recopie de §5.3 : `Mois, PosteCout, Montant`) ; un schéma divergent **refuse** l'écriture plutôt que de poser des cellules à l'aveugle. Un **test statique** (harnais garde-fous) casse en plus la CI si la projection serveur diverge du contrat (leçon S45, classe de bug de l'écart 3g).
- **Fil d'audit + post-vérification** — `proposition_id` est **tracé dans le retour** (d'où vient l'inscription) ; après l'`append`, la ligne est **relue** et rendue (`ligne_relue`). `Retry-After` honoré sur `429`/`503` (GET/POST). **Aucune suppression** (aucune primitive ; `supprimer_definitivement` reste proscrit).

**Correctif 0.20.1 (incident tenant du 01/08 — le mois RELU n'est pas le mois ÉCRIT).** À l'épreuve, l'écriture **réussissait** mais la **post-vérification** tombait en **FAUX-ROUGE** : l'outil écrit `Mois` en `« AAAA-MM-01 »` (ISO), mais l'**API Workbook restitue une cellule DATE en SÉRIAL Excel** (ex. `46204` pour `2026-07-01`, époque 1899-12-30) selon le formatage du classeur — la comparaison ISO ne matchait donc jamais la ligne relue, et la **garde d'idempotence portait le même biais** (risque de **doublon** à une seconde inscription). Même famille de piège que le **faux-vert « ouvrabilité à froid »** (§5.6, `T-0035`) : la forme **relue** n'est pas la forme **écrite**. Correctif : un **normaliseur unique `_mois_en_iso`** rend `« AAAA-MM-JJ »` depuis un **sérial Excel** (entier/flottant/chaîne), une **chaîne ISO** (avec ou sans heure) ou une **date Excel `m/j/aaaa`** — **fail-closed** (valeur inconvertible → `ValueError`, jamais de devinette ; le repli littéral buggé est **supprimé**). Il est appliqué aux **deux** comparaisons — garde d'idempotence **et** post-vérification —, **une seule fonction, deux usages** ; aucun autre chemin modifié. L'incident est **rejoué en régression CI** (mock restituant `[46204, "fonctionnement-reel", 706.84]` : la ligne est reconnue **et** une seconde inscription du mois est refusée).

Cran : **validé** (`table-des-crans.yaml` : `inscrire_cout_structure`) — écriture d'une donnée financière à **audience restreinte** qui alimente le pilotage (EBITDA §5.4), irréversible côté source : porte humaine avant l'action. La **mise en service** (pose de `GRAPH_REF_STRUCTURE_DRIVE_ID` / `GRAPH_REF_STRUCTURE_ITEM_ID` sur le service, build + déploiement de l'image **0.20.0**, correctif **0.20.1**) et l'**épreuve tenant** restent des gestes gardien après merge.

> **Hors de cette PR (signalé, non fait).** Cette PR ne porte **que** la primitive serveur. Sont des **PR ultérieures**, hors périmètre : le **skill de contrôle mensuel** (lecture de la boîte mail désignée, triple test de qualification des factures fournisseur, lissage, rapport d'écart, dépôt de la ligne candidate — cran `proposer_controle_structure`) ; la **lecture cockpit** de `T_Parametres` (forfait) et de `T_Structure` (réel). Et la **dette Workbook générale** (sessions Workbook persistantes, `$batch`, refonte globale du `Retry-After`) relève d'un **chantier 4b séparé** — **non touchée ici** : `inscrire_cout_structure` réutilise à l'identique les idiomes existants (client par appel, `Retry-After` borné local).
...
| `GRAPH_CRM_LIST_ID` | Identifiant de la Liste **« CRM »** — **cible figée** de `allouer_code_mission`, qui n'y touche que la colonne `CodeMission`. Identifiant **public**. Liste créée au runbook `T-0026` ; id posé au runbook `T-0038` ; absent = allocateur fermé (`ConfigManquante`). **Pas un secret.** |
| `GRAPH_FACTURES_LIST_ID` | Identifiant du registre **« Factures »** — **cible figée** de `allouer_num_facture` (source de vérité du NumFacture), **même site que « CRM »**. Identifiant **public**. Liste créée et id posé au runbook `T-0030` ; absent = allocateur de facture fermé (`ConfigManquante`). **Pas un secret.** |
| `GRAPH_REF_STRUCTURE_DRIVE_ID` | Identifiant du **drive** (site Contrats-et-administratif) hébergeant le référentiel de coûts — **cible figée** de `inscrire_cout_structure` (écrit `T_Structure`). Identifiant **public**. Posé au runbook gardien `T-0032` ; absent = inscription fermée (`ConfigManquante`). **Pas un secret.** |
| `GRAPH_REF_STRUCTURE_ITEM_ID` | Identifiant du **classeur** `referentiel-structure.xlsx` (« 07 - Coût de Structure ») — **seul classeur** écrit par `inscrire_cout_structure`, jamais choisi par l'appelant. Identifiant **public**. Posé au runbook gardien `T-0032` ; absent = inscription fermée (`ConfigManquante`). **Pas un secret.** |
