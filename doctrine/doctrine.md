...
> **Version** : 1.11 — *promu*. **Changelog** : v1.11 — **promu**, 1er août 2026 (S46, arbitrage gardien « merge vaut promotion ») : §7 — ajout d'une **note d'exécution** en fin de section (les huit points de la boucle sont inchangés) : dans l'implémentation GitHub, l'approbation-merge du gardien sur `main` EST les étapes 3-4 (approbation de la version précise + déplacement du pointeur canonique) ; il n'existe pas d'acte de promotion distinct du merge, et l'étiquette « candidat » ne désigne plus que l'état d'une PR ouverte. La porte humaine tient par construction (CODEOWNERS `/doctrine` et `/contrats/socle` ; chemins sensibles de l'avis d'impact `skills/`, surfaces de production, `backlog/chantiers/` — PR #291). Motif : depuis le 26/07 le second temps « re-étiquetage candidat → promu » n'était plus joué, et tout le canon récent faisait foi en production sous l'étiquette « candidat » — l'étiquette ne distinguait plus rien. **Mergé par le gardien = promu : la présente entrée applique sa propre règle.** Les étiquettes historiques des changelogs ne sont pas réécrites. Aucune autre section modifiée. v1.10 — **promu** via boucle de promotion, 11 juillet 2026 (toilettage S30-4.0) : §10 — retrait de l’auto-approbation du faible risque *contre politique* (`T-0020-c`) des évolutions restant à construire — éprouvée en production (PR #188 rouge skippé / PR #189 auto-mergée par l’App, agent-gardien v1) ; le tiret « Fédération de la promotion » ne conserve que la généralisation par domaine comme reste à construire ; §10 bis : clause d'auto-approbation passée à l'éprouvé. Aucune autre section modifiée. v1.9 — **promu** via boucle de promotion, 10 juillet 2026 (toilettage S29) : §10 — retrait du tiret « Plan de l’utilisateur quotidien / cockpit » (Tour de contrôle T-0014 soldé : construit, déployé tenant-wide, éprouvé sur listes M365 réelles) ; actualisation du tiret « Fédération de la promotion » pour acter la première délégation advenue (Communication, cf. §10 bis et registre organisation.md §4.1), la généralisation par domaine et l’auto-approbation du faible risque par l’agent-gardien restant au futur (T-0020-c à faire) ; ajout d’un tiret « Miroir de continuité du canon » (évolution connue non construite, cf. T-0029). Aucune autre section modifiée. v1.8 — **promu** via boucle de promotion, 5 juillet 2026 (toilettage) : §10 bis — la cadence prévisionnelle de fédération (trois sous-tirets par phase d'effectif) est remplacée par le principe + un renvoi au **registre des délégations `organisation.md` §4.1** (seule source de vérité du « qui promeut quoi ») ; acte la première délégation advenue dès Fondation (03/07/2026). Aucune autre section modifiée. v1.7 — **promu** via boucle de promotion, 19 juin 2026 : §10 — le tiret « Plan de l'utilisateur quotidien » actualisé — le cockpit (« Tour de contrôle ») passe de *à concevoir* à *conçu et cadré, reste à construire* (renvoi au contrat socle `contrats/socle/tour-de-controle.md` promu et au chantier `T-0014`). Le cockpit demeure dans §10 car non encore construit. Aucune autre section modifiée. v1.6 — **promu** via boucle de promotion, 12 juin 2026 (**décision du gardien — révision de la sortie de firme**) : (D1) la porte d'**anonymisation** se déclenche à la **communication grand public** (site, publication, institutionnel — tout canal non adressé à un client identifié dans une relation contractuelle), non plus à toute sortie vers un client : la **relation firme-client est considérée comme la firme**, un client n'est **jamais anonymisé pour lui-même** ; (D2) **nouvelle porte automatique** : la **matière issue d'un client tiers** est anonymisée **avant insertion** dans un livrable client (exigence NDA) ; (D3) un **humain collaborateur envoie librement** un livrable à son client — aucune validation tierce, aucun grade, aucune délégation : **les crans gouvernent les agents, pas les humains** ; (D4) l'**envoi d'un livrable au client par un AGENT** reste au cran *validé*, porte tenue par un **grade habilité** (grille `organisation.md` §4 bis, **requalifiée** : tenants de porte des actions d'agent engageant la firme vis-à-vis d'un client et de la communication grand public, non plus condition de l'envoi humain) ; (D5) la **communication grand public** reste au régime antérieur (cran *validé* + porte d'anonymisation). §6 (tableau et « qui tient la porte ») et §9 (confidentialité) alignés ; aucune autre section modifiée. v1.5 — **promu** via boucle de promotion, 10 juin 2026 (session stop-doctrine) : §9 — **principe zéro secret applicatif** (humains = identité Entra ; workloads = identité managée ou fédération d'identité ; tout secret subsistant = dette tracée avec échéance), **renvoi au contrat socle `contrats/socle/identites-et-secrets.md`** qui porte le modèle des identités appelantes et le registre des dettes. Aucun autre changement. v1.4 — **promu** via boucle de promotion (PR #25), 8 juin 2026 (session doctrine) : §8 — **extension de la chaîne d'autorité `guide → Claude → M365` aux APPAREILS** (parc / poste de travail : ABM, Intune, Entra), pas seulement aux droits des personnes ; une configuration de poste est un *dérivé* d'une décision de parc promue (`contrats/socle/parc-collaborateur.md`), réconciliée, jamais saisie à la main. **Arbitrage du gardien rendu EN FAVEUR de la ligne §8 (appareils)** ; statut aligné *candidat → promu*, contenu de la ligne **inchangé**. v1.3 — promu via boucle de promotion, 7 juin 2026 : §4 — étanchéité des deux plans explicitée (modifier une règle = PR ; travail métier = jamais de PR) ; §6 — porte de sortie de firme tenue par un rôle métier habilité (jamais au titre de gardien) + ligne du cran anonymisation alignée sur la « sortie externe » ; §9 — critère d'anonymisation aligné sur la « sortie externe » (fin de la logique « changement de destinataire / inter-client »). v1.2 — promu (contenu inchangé depuis le candidat v1.1). v1.1 — enrichie en lien avec la présentation « Talents & recrutement » (ajout du §10 bis).
...
| **validé** | irréversible **ou** sort de la firme **ou** rayon d'impact large | Porte humaine (gardien ou rôle désigné) avant l'action | Promouvoir un contrat socle ; envoi d'un livrable au client **par un agent** |
| **anonymisation** (porte automatique) | **communication grand public** ; **matière d'un client tiers** dans un livrable client — voir `anonymisation.md` §1 | Déclenchée par règle, sans humain | Publier sur le site un insight tiré d'une mission |
...
**Qui tient la porte de sortie de firme.** Un **collaborateur envoie librement son livrable à son client** : il est **responsable de son acte** ; aucun grade, aucune délégation, aucune validation tierce n'est requise. **Les crans gouvernent les agents, pas les humains.** Ce que la firme grade, c'est la **supervision d'une machine**, pas la confiance dans la personne : quand c'est un **agent** qui expédie un livrable au client, la porte humaine du cran *validé* est tenue par un **grade habilité** (grille concrète : `organisation.md` §4 bis). La **communication grand public** reste sous **porte validée** et **porte d'anonymisation**, tenant de porte selon la même grille. Cette tenue de porte relève du rôle **métier**, jamais du rôle de gardien. Qu'une même personne — le fondateur — porte les deux rôles ne les confond pas : il tient une porte **comme associé**, pas comme gardien ; ce qui reste exclu, c'est que le gardien intervienne dans une affaire **en tant que** gardien (règle d'or §4). La distinction porte sur le **rôle au titre duquel on agit**, pas sur la personne. Cette validation métier est **informelle** (mail, Teams, interface Claude) ; elle n'est **jamais** une PR GitHub. La doctrine en reste à la règle **abstraite** ; la **grille concrète** d'habilitation — quels grades, sous quelle délégation — vit dans `contrats/socle/organisation.md` (réalisation volatile, §2), jamais ici.
...
> **Note d'exécution (arbitrage gardien du 1er août 2026, S46).** Dans l'implémentation GitHub de cette boucle, l'approbation-merge du gardien sur `main` est les étapes 3-4 : merger un candidat au canon VAUT promotion. Il n'existe pas d'acte de promotion distinct du merge ; l'étiquette d'en-tête « candidat » est celle d'une PR ouverte, elle ne survit pas au merge. La porte humaine tient par construction : CODEOWNERS (`/doctrine`, `/contrats/socle`) et chemins sensibles de l'avis d'impact (`skills/`, surfaces de production, `backlog/chantiers/` — PR #291) garantissent qu'aucun artefact de canon n'atteint `main` sans geste du gardien (ou de son délégué, `organisation.md` §4.1). Les étiquettes historiques « candidat »/« promu » des changelogs ne sont pas réécrites.
...
> **Cette chaîne vaut aussi pour les APPAREILS, pas seulement pour les droits des personnes.** La configuration d'un poste de travail — son enrôlement (Apple Business Manager / Intune), ses politiques et applications déployées, son appartenance aux groupes d'enrôlement Entra — est elle aussi un *dérivé* d'une décision de **parc** promue dans le guide (`contrats/socle/parc-collaborateur.md`), projetée par réconciliation au moindre privilège dans M365/Intune, **jamais paramétrée à la main**. *Le dérivé n'est jamais le saisi* (§2) s'étend ainsi du droit d'accès à l'**appareil** lui-même : on change la politique de parc dans le guide, M365/Intune suit en conséquence — jamais l'inverse.

## 9. Confidentialité, conformité et qualité

- **Confidentialité.** Les données restent dans M365, sans isolation par mission. La protection passe par l'anonymisation, exécutable et tracée, déclenchée à la **communication grand public** et sur la **matière d'un client tiers** insérée dans un livrable client — et **non** sur la réutilisation inter-client interne, qui reste nominative, ni sur le livrable rendu au client pour ses propres données (§6 ; déclencheur complet : `contrats/socle/anonymisation.md` §1).
...
- **Zéro secret applicatif.** Les humains s'authentifient par leur identité Entra, les workloads par identité managée (ou fédération d'identité) ; aucun service ne détient ni ne distribue de secret applicatif. Tout secret subsistant est une dette tracée, avec échéance, au registre de `contrats/socle/identites-et-secrets.md`.

## 10. Évolutions connues (nommées, non encore construites)

Pour que le lecteur sache ce qui est délibérément reporté :

- **Fédération de la promotion.** À 50-200 collaborateurs, un promoteur humain unique sature. La première délégation par périmètre est advenue dès la Fondation (Communication) ; le « qui promeut quoi » — titulaires, périmètres, suppléances — vit au registre des délégations `contrats/socle/organisation.md` §4.1 (cf. §10 bis). Reste à construire : la généralisation de la délégation par domaine. Le gardien conserve le socle, délègue le reste.
- **Couche de résolution automatique** (§8) : à construire pour que la propagation soit instantanée sans intervention manuelle.
- **Miroir de continuité du canon.** Le dépôt de fondations est aujourd’hui le point unique de vérité *et* le point unique de défaillance. Un miroir de continuité — copie de secours gouvernée et restaurable du canon Git, en lecture seule — reste à concevoir et à construire (`T-0029`).
...
- **La trajectoire de la firme cadence la fédération de la promotion** (§10) — le principe est ici, l'état réel est au registre : le gardien conserve le socle et délègue le reste ; l'agent-gardien auto-approuve désormais le faible risque contre politique (éprouvé, PR #188/#189). Le « qui promeut quoi » — titulaires, périmètres, suppléances — vit dans le **registre des délégations `contrats/socle/organisation.md` §4.1**, seule source de vérité. La première délégation de promotion est advenue dès la phase *Fondation* (3 juillet 2026, contrat local communication) : la fédération suit le besoin réel, pas un cap d'effectif.
...
*Fin de la doctrine de gouvernance v1.7 (promu). Ce document est un contrat socle : il évolue par la boucle de promotion (§7), sous l'autorité du gardien du temple.*
